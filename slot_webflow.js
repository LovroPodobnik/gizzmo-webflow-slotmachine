...
var BLURB_TBL = ["No win!", "Bravo!", "Odličnoooooooo!", "JACKPOT!"];
...
function preloadImages(images, callback) {
  function _preload(asset) {
    ...
    asset.img.src =
      "https://gizzmo.si/assets/gizzmo/landings/emoji_jackpot/img/v6/" +
      asset.id +
      ".png?v=16";
  }
...
function SlotGame() {
    ...
    function _fill_canvas(canvas, items) {
      ...
      for (var i = 0; i < ITEM_COUNT; i++) {
        ...
        ctx.drawImage(
          asset.img,
          3,
          i * SLOT_HEIGHT + IMAGE_TOP_MARGIN,
          asset.img.width,
          asset.img.height
        );
        ctx.drawImage(
  ...
  $("#play").click(function (e) {
    ...
    game.restart();
    var audio = new Audio(
      "https://gizzmo.si/assets/gizzmo/landings/emoji_jackpot/audio/woo.mp3"
    );
    audio.play();
  });
...
function Game() {
  ...
  //    this.c1.css("width", "64px");
  //    this.c1.css("height", "64px");
...
Game.prototype.restart = function () {
  ...
  if (letThemWin == 2) {
    this.result1 = _find(this.items1, "gold-64");
    this.result2 = _find(this.items2, "goods-64");
    this.result3 = _find(this.items3, "gold-64");
  }
  if (letThemWin == 3) {
    this.result1 = _find(this.items1, "energy-64");
    this.result2 = _find(this.items2, "gold-64");
    this.result3 = _find(this.items3, "build-64");
  }
...
Game.prototype.update = function () {
  ...
  switch (this.state) {
    ...
    case 6: // check results
      ...
      document.getElementById("kolikoPopusta").innerHTML = value[letThemWin];
      document.getElementById("clipboardKopiraj").innerHTML = kpn[letThemWin];
      setCookie("lottery", letThemWin, 16);

      var sale_end = 1563926340000;

      var current_time = new Date();
      current_time = current_time.getTime();
      sale_end = sale_end - current_time;
      sale_end = sale_end / 1000 / 60 / 60 / 24;
      setCookie("auto_coupon", kpn[letThemWin], sale_end);
